...
import fs from 'fs'
import { Controller, FormattedResponse, VerifiedToken, File } from '../../interfaces'
import message_constants from '../../constants/message_constants'
import { User, Region, Role, Documents, Logs } from '../../db/models'
 ...
 */
export const provider_request_to_admin: Controller = async (req: Request, res: Response) => {
  try {
 ...
 */
export const provider_profile_view: Controller = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
    const token: string = authorization.split(' ')[1]
    const verified_token = jwt.verify(token, process.env.JWT_SECRET_KEY as string) as VerifiedToken
 ...
 */
export const provider_profile_reset_password: Controller = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
    const token: string = authorization.split(' ')[1]
    const verified_token = jwt.verify(token, process.env.JWT_SECRET_KEY as string) as VerifiedToken
...
export const provider_provider_profile_upload: Controller = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
    const token: string = authorization.split(' ')[1]
    const verified_token = jwt.verify(token, process.env.JWT_SECRET_KEY as string) as VerifiedToken
...
export const provider_myprofile_onboarding_upload = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
    const token: string = authorization.split(' ')[1]
    const verified_token = jwt.verify(token, process.env.JWT_SECRET_KEY as string) as VerifiedToken
    ...
    const provider_agreement_path =
      uploaded_files instanceof Array
        ? uploaded_files.find((file: File) => file.fieldname === 'provider_agreement')?.path
...
export const provider_myprofile_onboarding_view = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
    const token: string = authorization.split(' ')[1]
    const verified_token = jwt.verify(token, process.env.JWT_SECRET_KEY as string) as VerifiedToken
...
export const provider_myprofile_onboarding_delete = async (req: Request, res: Response) => {
  try {
