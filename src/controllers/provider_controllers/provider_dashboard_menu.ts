...
import fs from 'fs'
import { handle_request_state_physician } from '../../utils'
...
import { RequestModel, User, Notes, Documents, EncounterForm } from '../../db/models'
...
export const provider_accept_request: Controller = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
...
export const transfer_request_provider: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
...
export const view_notes_for_request_provider: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
    const formatted_response: FormattedResponse<any> = {
      status: true,
...
export const active_state_encounter: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
...
export const housecall: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
...
export const conclude_state_conclude_care: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
    const request = await RequestModel.findOne({
      where: {
    ...
    if (encounter_form) {
      if (encounter_form.is_finalize == 'true') {
        ...
        const formatted_request = {
          ...
          date_of_birth: encounter_form?.date_of_birth.toISOString().split('T')[0],
          date_of_service: encounter_form?.date_of_service?.toISOString().split('T')[0],
...
export const conclude_state_download_encounter_form: Controller = async (
  ...
  try {
    ...
    const generate_encounter_form_PDF = async (encounter_form: any) => {
      ...
      doc.text(`Confirmation Number: ${encounter_form.Request.confirmation_no}`, {
        align: 'left'
      })
      ...
      doc.text(`Date of birth: ${encounter_form?.date_of_birth.toISOString().split('T')[0]}`, {
        align: 'left'
      })
      doc.text(`Date of Service: ${encounter_form?.date_of_service.toISOString().split('T')[0]}`, {
        align: 'left'
      })
...
export const conclude_state_encounter_form: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
    const {
      first_name,
