...
import { User, RequestModel, Notes, Order, Documents, Logs } from '../../db/models'

export const patient_history: Controller = async (req: Request, res: Response) => {
  try {
...
export const patient_records: Controller = async (req: Request, res: Response) => {
  try {
    ...
    for (const request of requests) {
      const formatted_request = {
        sr_no: i,
        client_member: request.Patient?.firstname + ' ' + request.Patient?.lastname,
        created_at: request.createdAt,
        confirmation_no: request.confirmation_no,
        provider_name: request?.Physician?.firstname + ' ' + request?.Physician?.lastname,
...
export const patient_records_view_documents: Controller = async (req: Request, res: Response) => {
  try {
...
export const patient_records_view_case: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
    const formatted_response: FormattedResponse<any> = {
      status: true,
...
export const search_records: Controller = async (req: Request, res: Response) => {
  try {
    const {
      request_status,
      patient_name,
    ...
    const where_clause = {
      ...
      ...(from_date_of_service && {
        date_of_service: { [Op.gte]: from_date_of_service }
      }),
      ...(to_date_of_service && {
        date_of_service: { [Op.lte]: to_date_of_service }
      }),
      ...(request_status && {
...
export const search_record_delete: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
...
export const logs_history: Controller = async (req: Request, res: Response) => {
  try {
    const {
      type_of_log,
      search_by_role,
...
export const cancel_and_block_history: Controller = async (req: Request, res: Response) => {
  try {
    const { type_of_history, name, date, email, phone_no, page, page_size } = req.query as {
      [key: string]: string
      date: any
    }
    const page_number = Number(page) || 1
    ...
    const { count: total_count, rows: requests } = await RequestModel.findAndCountAll({
      ...
      where: {
        ...
        request_status: { [Op.like]: `%${type_of_history}%` }
      },
      include: [
        {
          model: User,
          as: 'Patient',
          attributes: ['user_id', 'firstname', 'lastname', 'email', 'mobile_no', 'status'],
          where: {
            ...(name && {
              [Op.or]: [
...
export const block_history_unblock: Controller = async (req: Request, res: Response) => {
  try {
    const { confirmation_no } = req.params
