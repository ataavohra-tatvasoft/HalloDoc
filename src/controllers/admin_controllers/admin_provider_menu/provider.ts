...
import { Controller, FormattedResponse, File, UserAttributes } from '../../../interfaces'
...
export const provider_list: Controller = async (req: Request, res: Response) => {
  try {
...
export const stop_notification: Controller = async (req: Request, res: Response) => {
  try {
    ...
    for (const user_id of user_ids) {
      ...
      if (!user) {
        return res.status(404).json({ message: message_constants.UNF + ' for ' + user_id })
      }
    }
...
export const contact_provider: Controller = async (req: Request, res: Response) => {
  try {
...
export const view_edit_physician_account: Controller = async (req: Request, res: Response) => {
  try {
    const { user_id } = req.params
    const formatted_response: FormattedResponse<any> = {
      status: true,
...
export const physician_account_reset_password: Controller = async (req: Request, res: Response) => {
  try {
    const {
...
export const save_user_information: Controller = async (req: Request, res: Response) => {
  try {
    const {
      user_id,
      username,
...
export const delete_provider_account: Controller = async (req: Request, res: Response) => {
  {
    try {
      const { user_id } = req.params
...
export const provider_profile_upload = async (req: Request, res: Response) => {
  try {
    const { user_id } = req.params
...
export const provider_onboarding_upload = async (req: Request, res: Response) => {
  try {
    ...
    const independent_contractor_agreement_path =
      uploaded_files instanceof Array
        ? uploaded_files.find((file: File) => file.fieldname === 'independent_contractor_agreement')
            ?.path
    ...
    const non_disclosure_path =
      uploaded_files instanceof Array
        ? uploaded_files.find((file: File) => file.fieldname === 'non_disclosure')?.path
    ...
    const update_document = async (document_name: string, document_path: string) => {
...
export const provider_onboarding_view = async (req: Request, res: Response) => {
  try {
    const { user_id } = req.params
    const {
      independent_contractor_agreement,
...
export const provider_onboarding_delete = async (req: Request, res: Response) => {
  try {
...
export const create_provider_account_refactored: Controller = async (
  ...
  try {
    ...
    const profile_picture_path = get_file_path(uploaded_files, 'profile_picture')
    const signature_photo_path = get_file_path(uploaded_files, 'signature_photo')
    const independent_contractor_agreement_path = get_file_path(
      uploaded_files,
      'independent_contractor_agreement'
    )
    const background_check_path = get_file_path(uploaded_files, 'background_check')
    ...
    await update_document(user.user_id, 'background_check', background_check_path)
...
export const common_save_provider_account: Controller = async (req: Request, res: Response) => {
  try {
    let {
      body: {
        user_id,
    ...
    if (region_ids) {
      await update_region_mapping(is_user.user_id, region_ids, req, res)
    }
    ...
    await update_document(is_user.user_id, 'background_check', background_check_path)
    await update_document(is_user.user_id, 'HIPAA', HIPAA_path)
    await update_document(is_user.user_id, 'non_disclosure', non_disclosure_path)
