 ...
 */
export const patient_profile_view: Controller = async (req: Request, res: Response) => {
  try {
...
export const patient_profile_edit: Controller = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
    const token: string = authorization.split(' ')[1]
    const verified_token = jwt.verify(token, process.env.JWT_SECRET_KEY as string) as VerifiedToken
    ...
    const { type, firstname, lastname, date_of_birth, mobile_no, email, street, city, state, zip } =
      req.body
...
export const create_patient_account: Controller = async (req: Request, res: Response) => {
  try {
