...
import { RequestModel, User, Role, Notes, Access, Region, Profession } from '../../db/models'
import { handle_request_state_exports, handle_request_state_physician_exports } from '../../utils'

/** Regions API */
export const region_with_thirdparty_API: Controller = async (req: Request, res: Response) => {
  try {
...
export const region_for_request_states: Controller = async (req: Request, res: Response) => {
  try {
...
export const transfer_request_regions: Controller = async (req: Request, res: Response) => {
  try {
    const formatted_response: FormattedResponse<any> = {
      status: true,
      data: []
...
export const regions: Controller = async (req: Request, res: Response) => {
  try {
    const formatted_response: FormattedResponse<any> = {
      status: true,
      data: []
...
/** Professions API */
export const professions_for_send_orders: Controller = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({ status: 'Successfull', professions: professions })
  } catch (error) {
    return res.status(500).json({ error: message_constants.ISE })
  }
}
export const professions: Controller = async (req: Request, res: Response) => {
  {
    try {
      const formatted_response: FormattedResponse<any> = {
        status: true,
...
export const export_single: Controller = async (req: Request, res: Response) => {
  try {
    ...
    for (const request of formatted_response.data) {
      const rowData = [
        ...
        request?.requestor_data?.first_name + ' ' + request?.requestor_data?.last_name,
...
export const export_all: Controller = async (req: Request, res: Response) => {
  try {
...
export const export_single_physician: Controller = async (req: Request, res: Response) => {
  try {
...
export const export_all_physician: Controller = async (req: Request, res: Response) => {
  try {
    const { authorization } = req.headers as { authorization: string }
    const token: string = authorization.split(' ')[1]
...
export const export_records: Controller = async (req: Request, res: Response) => {
  try {
    const {
      request_status,
      patient_name,
    ...
    for (const request of requests) {
      ...
      const formatted_request = {
        ...
        date_of_service: request.date_of_service
          ...
          : null,
        closed_date: request.closed_date ? request.closed_date.toISOString().split('T')[0] : null,
        email: request.Patient.email,
        phone_no: request.Patient.mobile_no,
        address:
          request.Patient.address_1 + ' ' + request.Patient.address_2 + ' ' + request.Patient.state,
        zip: request.Patient.zip,
        request_status: request.request_status,
        physician: request.Physician.firstname + ' ' + request.Physician.lastname,
        physician_note: physician_note?.description || null,
        cancelled_by_provider_note: cancelled_by_provider_note?.description || null,
    ...
    const column_widths = [25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25] // Adjust widths as needed
    worksheet.getColumn(1).width = column_widths[0]
    worksheet.getColumn(2).width = column_widths[1]
    worksheet.getColumn(3).width = column_widths[2]
    worksheet.getColumn(4).width = column_widths[3]
...
/**Action's API */
export const actions: Controller = async (req: Request, res: Response) => {
  try {
...
/**Physician's API */
export const physicians: Controller = async (req: Request, res: Response) => {
  {
    try {
      const formatted_response: FormattedResponse<any> = {
        status: true,
...
export const create_shift_region_physicians: Controller = async (req: Request, res: Response) => {
  try {
...
/**Role's API */
export const roles: Controller = async (req: Request, res: Response) => {
  {
    try {
...
/**Access ID's */
export const access: Controller = async (req: Request, res: Response) => {
  {
    try {
      const { account_type } = req.query as {
        [key: string]: string
